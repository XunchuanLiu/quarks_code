...
def getspwids_for_inmsfile(inmsfile,sourceconfig):
    ...
    for i in sourceconfig['wideband_outids']:
        selectspwids.append(sourceconfig['widebands'][i][inmsfile])  
            ...
            for j in allspwids:
                if spwinfo[j]['NumChan']>3000: 
                    ...
                    if np.abs(infreq-outfreq)<1000:
                        assert inmsfile not in sourceconfig['widebands'][i]
                        sourceconfig['widebands'][i][inmsfile] = '%i' % (spwinfo[j]['SpectralWindowId'])
